...
export function checkAdmin(adminCode) {
  return process.env.ADMIN_CODE && adminCode === process.env.ADMIN_CODE;
}
...
export async function getSummary() {
  ...
  const rawParticipants = participantsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const drawComplete = participants.length > 0 && participants.every(p => p.assignedToId && p.secretFriendId);
