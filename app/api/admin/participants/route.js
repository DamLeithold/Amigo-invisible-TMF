...
import { checkAdmin, generateCode, getSummary } from "../_utils";
...
export async function POST(request) {
  try {
    ...
    if (cleanNames.length < 3) return NextResponse.json({ error: "Cargá al menos 3 participantes." }, { status: 400 });
    ...
    return NextResponse.json({ ok: true, message: "Participantes guardados correctamente.", ...summary });
  } catch (error) {
